...
export default function AdminSaleContractClauses() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => firebaseApi.entities.SaleContractClause.create(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['sale-contract-clauses'] }); close(); },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => firebaseApi.entities.SaleContractClause.update(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['sale-contract-clauses'] }); close(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => firebaseApi.entities.SaleContractClause.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['sale-contract-clauses'] }),
  });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold">{L('بنود عقود البيع', 'بەندەکانی گرێبەستی فرۆشتن')}</h1>
          <p className="text-sm text-muted-foreground">{L('إدارة البنود والشروط التي تظهر افتراضياً في عقود البيع', 'بەڕێوەبردنی بەندەکانی بنەڕەت کە لە گرێبەستەکانی فرۆشتندا دەردەکەون')}</p>
        </div>
      ...
      {clauses.length === 0 && !showForm ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">{L('أضف البنود الافتراضية لعقود البيع', 'بەندەکانی بنەڕەت بۆ گرێبەستەکانی فرۆشتن زیاد بکە')}</p>
